...
export default function GroupsScreen() {
  ...
  useEffect(() => {
    if (authReady && userId) fetchGroups();
    else if (authReady && !userId) setLoading(false);
  }, [authReady, userId, fetchGroups]);
  ...
  return (
    <View style={[styles.root, { backgroundColor: colors.background }]}>
      ...
      {/* ── Overlays (no native Modal — avoids NativeTabs rendering conflicts on iOS) ── */}
      {modalMode !== 'none' && (
        <View style={styles.overlay}>
          ...
          {modalMode === 'setName' && (
            <View style={[styles.dialog, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Ionicons name="person-circle-outline" size={40} color={colors.primary} style={{ alignSelf: 'center' }} />
              <Text style={[styles.dialogTitle, { color: colors.foreground, fontFamily: 'Inter_700Bold', textAlign: 'center' }]}>
                What should we call you?
              </Text>
              <Text style={[styles.dialogSub, { color: colors.mutedForeground, fontFamily: 'Inter_400Regular', textAlign: 'center' }]}>
              ...
              {actionError ? <Text style={[styles.error, { color: colors.primary }]}>{actionError}</Text> : null}
...
const styles = StyleSheet.create({
  ...
  // In-view overlay (replaces Modal to avoid NativeTabs rendering conflicts)
  overlay: {

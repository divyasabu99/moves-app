...
export default function AuthScreen() {
  ...
  const switchMode = (m: Mode) => { setMode(m); setError(''); Haptics.selectionAsync(); };
      ...
      >
        ...
        <View style={[styles.tabs, { backgroundColor: colors.muted, borderRadius: 14 }]}>
          {(['login', 'register'] as Mode[]).map(m => (
            <TouchableOpacity key={m} onPress={() => switchMode(m)} activeOpacity={0.8}
              style={[styles.tab, mode === m && { backgroundColor: colors.card, borderRadius: 11 }]}
            >
        ...
        <View style={styles.form}>
          {mode === 'register' && (
            <View style={styles.fieldWrap}>
              <Text style={[styles.label, { color: colors.mutedForeground, fontFamily: 'Inter_500Medium' }]}>YOUR NAME</Text>
              <View style={[styles.inputRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <Ionicons name="person-outline" size={16} color={colors.mutedForeground} />
                <TextInput value={displayName} onChangeText={t => { setDisplayName(t); setError(''); }}
                  placeholder="How should we call you?" placeholderTextColor={colors.mutedForeground}
                  autoComplete="name" returnKeyType="next" onSubmitEditing={() => emailRef.current?.focus()}
          ...
          <View style={styles.fieldWrap}>
            ...
            <View style={[styles.inputRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <TextInput ref={emailRef} value={email} onChangeText={t => { setEmail(t); setError(''); }}
          ...
          <View style={styles.fieldWrap}>
            ...
            <View style={[styles.inputRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <TextInput ref={passwordRef} value={password} onChangeText={t => { setPassword(t); setError(''); }}
          ...
          {mode === 'register' && (
            <View style={styles.fieldWrap}>
              ...
              <View style={[styles.inputRow, { backgroundColor: colors.card, borderColor: colors.border }]}>
                <Ionicons name="lock-closed-outline" size={16} color={colors.mutedForeground} />
                <TextInput ref={confirmRef} value={confirmPassword} onChangeText={t => { setConfirmPassword(t); setError(''); }}
          ...
          >
            {loading ? <ActivityIndicator color={colors.primaryForeground} /> : (
              <>
                <Ionicons name={mode === 'login' ? 'log-in-outline' : 'person-add-outline'} size={18} color={colors.primaryForeground} />
...
const styles = StyleSheet.create({
  ...
  tab: { flex: 1, paddingVertical: 10, alignItems: 'center', shadowColor: '#000', shadowOpacity: 0.06, shadowRadius: 4, shadowOffset: { width: 0, height: 1 }, elevation: 1 },
  tabText: { fontSize: 14 },
  form: { gap: 16 },
  fieldWrap: { gap: 7 },
  label: { fontSize: 11, letterSpacing: 1.2 },
  inputRow: { flexDirection: 'row', alignItems: 'center', gap: 10, borderRadius: 14, borderWidth: 1, paddingHorizontal: 14, paddingVertical: 13 },
